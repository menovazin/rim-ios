...
/// Paginated Locations list reducer matching `locations_page.dart`.
///
/// Mirrors `CharactersReducer`/`EpisodesReducer`'s pagination shape exactly.
@Reducer
public struct LocationsReducer {
    @ObservableState
    public struct State: Equatable, Sendable {
        public var items: IdentifiedArrayOf<Location>
        public var page: Int
        ...
        public init(
            items: IdentifiedArrayOf<Location> = [],
            page: Int = 1,
            hasNext: Bool = true,
    ...
    public enum Action: Equatable, Sendable {
        case onAppear
        case loadInitial
        case loadInitialResponse(Result<PageResult<Location>, NSError>)
        case loadMore
        case loadMoreResponse(Result<PageResult<Location>, NSError>)
        case retry
        case cardTapped(Location)
    }
    ...
    public var body: some ReducerOf<Self> {
        Reduce { state, action in
            ...
            case .loadInitial:
                ...
                return .run { send in
                    do {
                        let result = try await apiClient.fetchLocations(page)
            ...
            case .loadMore:
                ...
                return .run { send in
                    do {
                        let result = try await apiClient.fetchLocations(nextPage)
